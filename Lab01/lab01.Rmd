...
You can find all the material for these tutorials [here!](https://github.com/angeella/summer_school_2022_VERA) 
...
`R` provides a number of packages (**packages**) that contain very useful functions of various kinds. For example with the following commands:
...
`r emo::ji("boom")` _Please note_: find errors/bugs on google or [stackoverflow](https://stackoverflow.com)!
...
`r emo::ji("boom")` _Please note_: The symbols ``=`` and ``<-`` are used to assign a value to a variable
...
Let see some examples together:
...
\[
  \text{HappinessScore}_i \approx \beta_0 + \beta_1 \, \text{Beer_PerCapita}_i + \beta_2 \text{HDI}_i + \dots + \beta_{3} \text{Wine_PerCapita}_i, \qquad i = 1, \dots, n.
\]
