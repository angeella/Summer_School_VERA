...
We will use the packages `igraph` and `ggraph`. The difference between these two packages is that `ggraph` is based on `ggplot` structure/language, while `igraph` uses basic `R` language. Both are useful, you can use the one that best fits with you!
...
`r emo::ji("boom")` **Please note**! Here we fix the seed using the function `set.seed(x)` since the default `layout` used by the `plot.igraph` function is `layout_nicely`, a smart function that chooses a layouter based on the graph. Some layout algorithm are not deterministic. For further information, see `?layout_nicely`.
...
`r emo::ji("boom")` **Please note**! You can find the full list of network layout available typing `?layout`.

## Communities

We can also try to make the network map more useful by showing the communities within it. So, let take the communities computed by `cluster_louvain` in the previous laboratory (i.e., `gr` objects):
...
We can do everything done with the `igraph` package using the `ggraph` package; however, the layout is generally fancier, especially for a large neural network.
